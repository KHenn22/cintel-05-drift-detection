"""
hennelly_drift_detector_percentage.py - Project script (example).
...
Paths (relative to repo root)
    ...
    INPUT FILE: data/reference_metrics_hennelly.csv
    INPUT FILE: data/current_metrics_hennelly.csv
    OUTPUT FILE: artifacts/drift_summary_hennelly_percentage.csv
    OUTPUT FILE: artifacts/drift_summary_long_hennelly_percentage.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.hennelly_drift_detector_percentage
...
REFERENCE_FILE: Final[Path] = DATA_DIR / "reference_metrics_hennelly.csv"
CURRENT_FILE: Final[Path] = DATA_DIR / "current_metrics_hennelly.csv"

OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "drift_summary_hennelly_percentage.csv"
SUMMARY_LONG_FILE: Final[Path] = (
    ARTIFACTS_DIR / "drift_summary_long_hennelly_percentage.csv"
)
...
def main() -> None:
    ...
    # Added median values as well to provide more context about the distribution
    # of data.

    reference_summary_df = reference_df.select(
        [
            pl.col("requests").mean().alias("reference_avg_requests"),
            pl.col("requests").median().alias("reference_median_requests"),
            pl.col("errors").mean().alias("reference_avg_errors"),
            pl.col("errors").median().alias("reference_median_errors"),
            pl.col("total_latency_ms").mean().alias("reference_avg_latency_ms"),
            pl.col("total_latency_ms").median().alias("reference_median_latency_ms"),
        ]
    )
    ...
    current_summary_df = current_df.select(
        [
            ...
            pl.col("requests").median().alias("current_median_requests"),
            pl.col("errors").mean().alias("current_avg_errors"),
            pl.col("errors").median().alias("current_median_errors"),
            pl.col("total_latency_ms").mean().alias("current_avg_latency_ms"),
            pl.col("total_latency_ms").median().alias("current_median_latency_ms"),
        ]
    )
    ...
    #
    # Both mean and median differences are calculated.
    # Mean differences show the overall average shift.
    # Median differences show the shift in the middle of the distribution,
    # which can help identify changes in typical behavior that may be masked by outliers.
    ...
    requests_median_difference_recipe: pl.Expr = (
        (pl.col("current_median_requests") - pl.col("reference_median_requests"))
        .round(2)
        .alias("requests_median_difference")
    )

    errors_median_difference_recipe: pl.Expr = (
        (pl.col("current_median_errors") - pl.col("reference_median_errors"))
        .round(2)
        .alias("errors_median_difference")
    )

    latency_median_difference_recipe: pl.Expr = (
        (pl.col("current_median_latency_ms") - pl.col("reference_median_latency_ms"))
        .round(2)
        .alias("latency_median_difference_ms")
    )
    ...
    drift_df: pl.DataFrame = combined_df.with_columns(
        [
            ...
            requests_median_difference_recipe,
            errors_median_difference_recipe,
            latency_median_difference_recipe,
    ...
    # Two sets of drift flags are defined.
    # Mean-based flags show when the overall average has shifted meaningfully.
    # Median-based flags show when the typical behavior has shifted meaningfully,
    # which can be important even if the average is stable due to outliers.
    # Both sets use the same thresholds for consitency.
    ...
    requests_median_is_drifting_flag_recipe: pl.Expr = (
        pl.col("requests_median_difference").abs() > REQUESTS_DRIFT_THRESHOLD
    ).alias("requests_median_is_drifting_flag")

    errors_median_is_drifting_flag_recipe: pl.Expr = (
        pl.col("errors_median_difference").abs() > ERRORS_DRIFT_THRESHOLD
    ).alias("errors_median_is_drifting_flag")

    latency_median_is_drifting_flag_recipe: pl.Expr = (
        pl.col("latency_median_difference_ms").abs() > LATENCY_DRIFT_THRESHOLD
    ).alias("latency_median_is_drifting_flag")
    ...
    drift_df = drift_df.with_columns(
        [
            ...
            requests_median_is_drifting_flag_recipe,
            errors_median_is_drifting_flag_recipe,
            latency_median_is_drifting_flag_recipe,
        ]
    )
    ...
    LOG.info("========================")
    LOG.info("1. Summarize each period with means and medians.")
    LOG.info("2. Compute difference of means and medians.")
    LOG.info(
        "3. Flag drift if absolute difference of means or medians exceeds a threshold."
    )
    LOG.info("========================")
